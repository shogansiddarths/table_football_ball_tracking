...
ball_confidence = 0
INVERSE_RODS = False  # True if Pi camera rods mapping is flipped
...
def get_player_color_by_rod(rod_index):
    white_rods = [3,5,7,8]
    if INVERSE_RODS:
        white_rods = [i for i in range(1,9) if i not in white_rods]
    return "white" if rod_index in white_rods else "black"

# ---------------- PROCESS FRAME ----------------
def process_frame(frame):
    global possession_counts,last_print_time,frame_counter, prev_ball_pos

    frame_counter = getattr(process_frame,"frame_counter",0) +1
    process_frame.frame_counter = frame_counter
    if frame_counter%FRAME_SKIP!=0: return None

    hsv,gray,frame = preprocess(frame)
    ball_center = detect_ball_pi(hsv, prev_ball_pos)
    players = detect_players(hsv,gray)

    rod_ys = detect_rods_dynamic(frame)
    if rod_ys: players = assign_players_to_rods(players,rod_ys)

    # Assign colors using rods
    ...
    # Narrow field HSV range to remove white lines
    field_mask = cv2.inRange(hsv,np.array([30,50,50]),np.array([90,255,200]))
    field_mask = cv2.morphologyEx(field_mask,cv2.MORPH_CLOSE,np.ones((5,5),np.uint8))
...
def run_pi_camera():
    try: from picamera2 import Picamera2
        ...
        print("picamera2 not available"); return
    picam2 = Picamera2()
...
if __name__=="__main__":
    ...
    choice = input("Mode: ")
    if choice=="1": 
        run_image(r"C:\Users\Deepika\OneDrive\Documents\Deepika\balltracker2.jpg")
    elif choice=="2": 
        run_pi_camera()
    else: 
