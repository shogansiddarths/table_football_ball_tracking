...
# If Pi camera rods appear inverted, set True
INVERSE_RODS = False  
...
def detect_rods_dynamic(frame, angle_tol=15):
    ...
    if lines is not None:
        ...
        for l in lines:
            ...
            if abs(angle) <= angle_tol: horizontal_lines.append(l[0])
...
def get_player_color_by_rod(rod_index):
    # Default mapping: rods 3,5,7,8 = white
    white_rods = [3,5,7,8]
    if INVERSE_RODS:
...
def process_frame(frame):
    ...
    # Assign player colors dynamically
    for idx,p in enumerate(players):
        # Here rod index 1-8 (rough approximation)
        rod_idx = min(idx+1,8)
    ...
    if now - last_print_time >= 1.0:
        ...
        print(f"Speed: {speed:.2f} cm/s | Possession Code: {possession_code} | White: {white_pct:.1f}% | Black: {black_pct:.1f}%")
        last_print_time = now
